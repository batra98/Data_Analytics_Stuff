...
class Apriori():
	...
	def run(self,data_itr):
		...
		Cset = self.ItemsWithMinSupport(self.itemSet)
		k = 2
		while len(Cset) != 0:
			self.ans[k-1] = Cset
			Lset = self.joinSet(Cset,k)
			Cset = self.ItemsWithMinSupport(Lset)
			print(k)
			k = k+1

		for _,val in self.ans.items():
			self.realtionItems.extend([(tuple(item),float(self.freqSet[item])/len(self.transactionList)) for item in val])
